...
const AdminDashboard = () => {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      console.error(error);
      alert(
        "Image upload failed. Ensure you selected a valid image and the backend is running.",
      );
    } finally {
  ...
  const uniquePayingCustomers = new Set(
    paidConfirmedBookings.map(payerIdKey).filter(Boolean),
  ).size;
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
          <div>
            ...
            <h3 className="text-3xl font-black text-green-600">
              ₹
              {paidConfirmedBookings.reduce((sum, b) => sum + b.amount, 0)}
            </h3>
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
          <div>
            ...
            <h3 className="text-3xl font-black text-blue-600">
              {paidConfirmedBookings.length}
            </h3>
            <p className="text-gray-400 text-xs font-semibold mt-1">
              {uniquePayingCustomers} unique customer
              {uniquePayingCustomers !== 1 ? "s" : ""}
            </p>
          </div>
          ...
          >
              ...
              <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition relative overflow-hidden">
                {formData.image ? (
                  ...
                  <div className="flex flex-col items-center justify-center pt-5 pb-6 text-gray-500">
                    {isUploading ? (
                      <span>Uploading directly to Cloudinary... ☁️</span>
                    ) : (
                      <>
                        <svg
                        ...
                        <p className="text-xs">
                          PNG, JPG or WEBP (Standard Cloudinary formatting)
                        </p>
                      </>
                    )}
                  </div>
            ...
            <div className="md:col-span-2 flex justify-end">
              <button
                ...
                className="text-sm bg-indigo-100 text-indigo-700 font-bold py-2 px-4 rounded-lg hover:bg-indigo-200 transition"
              >
                {isGenerating
                  ? "Generating..."
